...
import sn.oas.facturation.features.connectionHistory.repository.ConnectionHistoryRepository;
...
public class ConnectionHistoryServiceImpl implements ConnectionHistoryService{
    ...
    public void saveConnectionLog(String username, String ip, String status) {
        StatusConnectionHistory statusEnum = null;
        if (status != null) {
            try {
                statusEnum = StatusConnectionHistory.valueOf(status.trim().toUpperCase());
            } catch (IllegalArgumentException e) {
                statusEnum = StatusConnectionHistory.FAILED;
            }
        }
