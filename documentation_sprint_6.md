# Documentation Portail Client — Sprint 6

Cette documentation présente l'ensemble des développements et des APIs mis en place pour le **Portail Client (Sprint 6)** destiné à l'application mobile Flutter.
...
Le serveur Spring Boot tourne sur le port **`9090`** (défini dans [application-dev.yml](file:///d:/JBA/BACK/oas-back/oas-back/src/main/resources/application-dev.yml)).

Vous pouvez accéder à la documentation interactive Swagger via les URLs suivantes :
* **Page HTML (Swagger UI)** : [http://localhost:9090/swagger-ui/index.html](http://localhost:9090/swagger-ui/index.html)
* **Redirection automatique** : [http://localhost:9090/swagger-ui.html](http://localhost:9090/swagger-ui.html)
* **Spécification OpenAPI (JSON)** : [http://localhost:9090/v3/api-docs](http://localhost:9090/v3/api-docs)

> [!NOTE]
> En cas d'erreur de connexion à Swagger UI, vérifiez que l'application Spring Boot a démarré sans erreur et que vos variables de base de données PostgreSQL dans votre fichier `.env` à la racine sont correctement configurées.
